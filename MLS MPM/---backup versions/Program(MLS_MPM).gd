...
func _on_Program_ready():
	...
	for wall in barriers['window outline'].keys():
		...
		if wall == 'top':
			barriers['window outline'][wall]['velocity'] = Vector2(0.0,0.0)
			#barriers['window outline'][wall]['velocity'] = get_tree().get_root().get_node("Test Area/Simulation/Substance").maintain_velocity
			
		if wall == 'right':
			barriers['window outline'][wall]['velocity'] = Vector2(-0.0,0.0)
			#barriers['window outline'][wall]['velocity'] = Vector2(-get_tree().get_root().get_node("Test Area/Simulation/Substance").maintain_velocity.x,get_tree().get_root().get_node("Test Area/Simulation/Substance").maintain_velocity.y)
		...
		barriers['window outline'][wall]['momentum'] = barriers['window outline'][wall]['mass'] * barriers['window outline'][wall]['velocity']
	...
	for particle in material:
		...
		#	print(weight_interpolation,' wip')
			...
			if particle.within_range.back() == other_particle:
				c_cell_coefficient =  snapped((basis_coefficient / pow(grid_domain,2.0)),2.0)
		...
		particle.position.x = clampf(particle.position.x,(0+(grid_domain/2.0)),(ProjectSettings.get_setting('display/window/size/width')-(grid_domain/2.0) ))
		
		particle.position.y = snapped((particle.position.y + (time_passed * particle.velocity.y)),.01)
		particle.position.y = clampf(particle.position.y,(0+(grid_domain/2.0)),(ProjectSettings.get_setting('display/window/size/height')-(grid_domain/2.0) ))
		...
		#print(particle.F,' deformation gradient updated')
